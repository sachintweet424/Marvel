...
class CharacterListViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.listViewModel.getCharacterList()
    }
}
...
extension CharacterListViewController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.listViewModel.characterModel?.data?.results?.count ?? 0
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.bindData(model: self.listViewModel.characterModel?.data?.results?[indexPath.row])
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        viewControllerInstance.detailViewModel = CharacterDetailViewModel.init(id: "\(self.listViewModel.characterModel?.data?.results?[indexPath.row].id ?? 0)")
